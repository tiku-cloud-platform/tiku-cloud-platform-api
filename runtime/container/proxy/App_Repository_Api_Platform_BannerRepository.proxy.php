...
class BannerRepository implements ApiRepositoryInterface
{
    ...
    {
        $items = $this->bannerModel::query()->with(['coverFileInfo:uuid,file_url,file_name'])->where($closure)->where([['is_show', '=', 1]])->select($this->bannerModel->searchFields)->orderByDesc('orders')->paginate((int) $perSize);
