...
class MenuRepository implements ApiRepositoryInterface
{
    ...
    {
        $items = $this->menuModel::query()->with(['coverFileInfo:uuid,file_url,file_name'])->where([['is_show', '=', 1]])->where($closure)->select($this->menuModel->searchFields)->orderByDesc('orders')->paginate((int) $perSize);
