...
use App\Repository\Api\Rank\ScoreRepository;
...
class ScoreService implements ApiServiceInterface
{
    ...
    {
        return $this->scoreRepository->repositorySelect(self::searchWhere((array) $requestParams), (int) $requestParams['number'] ?? 10);
    }
    /**
     * 创建数据
     *
    ...
    public function serviceGroup(array $requestParams) : array
    {
        ...
        if (empty($cacheInfo)) {
            $cacheInfo = $this->scoreRepository->repositoryGroup(self::searchWhere((array) $requestParams), (int) $requestParams['size'] ?? 15);
