...
class WeChatApiRepository implements ApiRepositoryInterface
{
    ...
    {
        if ($this->userModel::query()->create($insertInfo)) {
            return true;
        }
        return false;
    }
    ...
    {
        $bean = $this->userModel::query()->with(['user:uuid,login_token'])->where([['is_forbidden', '=', 1]])->where($closure)->first($this->userModel->searchFields);
        if (!empty($bean)) {
            return $bean->toArray();
        }
        return [];
