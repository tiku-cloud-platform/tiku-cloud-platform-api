...
class MessageCategoryRepository implements ApiRepositoryInterface
{
    ...
    {
        $items = $this->categoryModel::query()->with(['coverFileInfo:uuid,file_url,file_name'])->where([['is_show', '=', 1]])->where($closure)->select($this->categoryModel->searchFields)->orderByDesc('orders')->paginate((int) $perSize);
