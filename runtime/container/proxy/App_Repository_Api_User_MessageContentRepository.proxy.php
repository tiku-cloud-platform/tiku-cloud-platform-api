...
class MessageContentRepository implements ApiRepositoryInterface
{
    ...
    {
        $items = $this->contentModel::query()->with(['category:uuid,title'])->where($closure)->select($this->contentModel->listSearchFields)->orderByDesc('id')->paginate((int) $perSize);
    ...
    {
        $bean = $this->contentModel::query()->with(['category:uuid,title'])->where([['is_show', '=', 1]])->where($closure)->first($this->contentModel->searchFields);
        if (!empty($bean)) {
            return $bean->toArray();
        }
        return [];
