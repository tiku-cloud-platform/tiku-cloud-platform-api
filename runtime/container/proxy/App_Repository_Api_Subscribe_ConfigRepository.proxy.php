...
class ConfigRepository implements ApiRepositoryInterface
{
    ...
    {
        $items = $this->configModel::query()->with(['coverFileInfo:uuid,file_url,file_name'])->where($closure)->select($this->configModel->searchFields)->orderByDesc('id')->paginate((int) $perSize);
