...
class WeChatUserService implements ApiServiceInterface
{
    ...
    public function serviceUpdate(array $requestParams) : int
    {
        ...
        $updateResult = $this->userRepository->repositoryUpdate((array) [['uuid', '=', $userInfo['uuid']], ['store_uuid', '=', $userInfo['store_uuid']]], (array) $requestParams);
        if ($updateResult) {
            // 更新缓存信息
            ...
            if (RedisClient::update((string) CacheKey::USER_LOGIN_PREFIX, (string) (new UserInfo())->getWeChatLoginToken(), (array) $userInfo, (int) CacheTime::USER_LOGIN_EXPIRE_TIME)) {
                return 1;
            }
            return 0;
        }
    ...
    public function serviceFind(array $requestParams) : array
    {
        ...
        $userInfo = ['sign' => ['is_sign' => 0, 'sign_days' => 0], 'collection' => 0, 'attention' => 0, 'score' => 0, 'unread_message' => 0, 'userInfo' => $userCacheInfo, 'favorites' => 0];
        ...
        // 答卷情况
        $userInfo['collection'] = (new ExamSubmitHistoryService())->serviceSubmitCount((array) ['user_uuid' => $userCacheInfo['user_uuid']]);
        // 签到情况
        ...
        // 登录状态
        $userInfo['sign'] = ['is_sign' => empty($userSignHistory) ? 0 : 1, 'sign_days' => empty($userSignCollection) ? 0 : $userSignCollection['sign_number']];
            ...
            try {
                if ($info['code'] == 0 && !isset($info['data']['errcode'])) {
                    ...
                    if (!empty($miniUserInfo)) {
                        // 更新用户
                        $result = $this->userRepository->repositoryUpdate((array) [['openid', '=', $miniUserInfo['openid']], ['store_uuid', '=', (new WeChatClient())->getUUIDHeader()]], (array) ['nickname' => $requestParams['userInfo']['nickName'], 'avatar_url' => $requestParams['userInfo']['avatarUrl'], 'gender' => $requestParams['userInfo']['gender'], 'country' => $requestParams['userInfo']['country'], 'province' => $requestParams['userInfo']['province'], 'city' => $requestParams['userInfo']['city'], 'language' => $requestParams['userInfo']['language']]);
                        (new StorePlatformUserService())->serviceUpdate((array) ['login_token' => $loginToken, 'uuid' => $miniUserInfo['user']['uuid'], 'store_platform_user_group_uuid' => $userGroup['uuid']]);
                        ...
                        $userInfo = $miniUserInfo = [
                            ...
                            'language' => $requestParams['userInfo']['language'],
                            'real_name' => '',
                            // 特殊参数就给一个默认空，避免小程序端用户登录之后，在个人中心会看到null相关的信息。
                            'mobile' => '',
                            'address' => '',
                            'longitude' => null,
                            'latitude' => null,
                        ...
                        Db::transaction(function () use($miniUserInfo, $userInfo, $wechatUuid, &$result, $userId) {
                            ...
                            // 新用户注册送积分
                            (new ScoreHistoryService())->serviceCreate((array) ['scene' => 'wechat_register', 'data' => ['type' => 1, 'user_uuid' => $userId]]);
                            $result = true;
                        });
                        $miniUserInfo['created_at'] = date('Y-m-d H:i:s');
                    }
                    if ($result) {
                        //$userInfo['user_agent'] = $this->request->header('Api-Agent', null);
                        // 生成Redis缓存信息
