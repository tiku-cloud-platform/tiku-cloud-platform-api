...
class SignCollectionRepository implements ApiRepositoryInterface
{
    ...
    {
        $bean = $this->signCollectionModel::query()->where($closure)->select($this->signCollectionModel->searchFields)->first();
        if (!empty($bean)) {
            return $bean->toArray();
        }
        return [];
    ...
    public function repositoryUpdate(array $updateWhere, array $updateInfo) : int
    {
        ...
        if (empty($bean)) {
            // 不存在记录
            if ($this->signCollectionModel::query()->create(['uuid' => UUID::getUUID(), 'user_uuid' => $updateWhere['user_uuid'], 'sign_number' => 1, 'store_uuid' => $updateWhere['store_uuid'], 'is_show' => 1])) {
                return 0;
            }
            return 1;
        } else {
    ...
    {
        return $this->signCollectionModel->fieldIncr((string) $this->signCollectionModel->getTable(), (array) $updateWhere, (string) 'sign_number', (int) 1);
    }
}