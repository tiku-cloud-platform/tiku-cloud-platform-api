...
class SignHistoryService implements ApiServiceInterface
{
    ...
    public function serviceCreate(array $requestParams) : bool
    {
        ...
        return $this->signHistory->repositoryCreate((array) [
            ...
            // 昨日签到记录
            'score' => empty($scoreConfig) ? [] : ['score' => $scoreConfig['score'], 'title' => $scoreConfig['title'], 'user_uuid' => $userInfo['user_uuid'], 'store_uuid' => $userInfo['store_uuid'], 'type' => 1, 'uuid' => UUID::getUUID(), 'score_key' => 'wechat_day_sign'],
        ]);
    }
    /**
     * 更新数据
    ...
    public function serviceFind(array $requestParams) : array
    {
        if (!empty($requestParams['sign_date'])) {
            $result = (new RedisClient())->redisClient->getBit(CacheKey::USER_SING_DAY . $requestParams['user_uuid'], (int) str_replace('-', '', $requestParams['sign_date']));
            if ($result == 1) {
                return ['sign_date' => $result];
            }
        }
