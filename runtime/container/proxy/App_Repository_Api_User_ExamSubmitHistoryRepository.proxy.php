 ...
 */
class ExamSubmitHistoryRepository implements ApiRepositoryInterface
{
    ...
    {
        $items = $this->historyModel::query()->with(['collection:uuid,title,file_uuid'])->where($closure)->select(['uuid', 'exam_collection_uuid', Db::raw("sum(score) as score"), 'submit_time'])->groupBy(['exam_collection_uuid'])->paginate($perSize);
    ...
    public function repositoryCreate(array $insertInfo) : bool
    {
        ...
        Db::transaction(function () use(&$result, $insertInfo, $score) {
            // 清空之前该试卷的记录
            $this->historyModel::where([['exam_collection_uuid', '=', $insertInfo[0]['exam_collection_uuid']], ['user_uuid', '=', $insertInfo[0]['user_uuid']]])->delete();
            // 插入记录
            $this->historyModel->batchInsert((string) $this->historyModel->getTable(), (array) $insertInfo, (string) '', (string) '', (string) 'user');
            // 增加试卷答题数量
            (new CollectionRepository())->repositoryIncrField((array) [['uuid', '=', $insertInfo[0]['exam_collection_uuid']]]);
            // 增加积分记录
