...
class ReadingRepository implements ApiRepositoryInterface
{
    ...
    {
        $items = $this->readingModel::query()->whereHas('relationCollection', $closure)->where([['is_show', '=', 1]])->select($this->readingModel->listSearchFields)->paginate((int) $perSize);
    ...
    {
        $bean = $this->readingModel::query()->where($closure)->where([['is_show', '=', 1]])->first(['uuid', 'title', 'content']);
        if (!empty($bean)) {
            return $bean->toArray();
        }
        return [];
