...
class OptionRepository implements ApiRepositoryInterface
{
    ...
    {
        $items = $this->optionModel::query()->with(['coverFileInfo:uuid,file_url,file_name'])->with(['optionItem:uuid,title,option_uuid,check,is_check'])->whereHas('relationCollection', $closure)->where([['is_show', '=', 1]])->select($this->optionModel->listSearchFields)->paginate((int) $perSize);
