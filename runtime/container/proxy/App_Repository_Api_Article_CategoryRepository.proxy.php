...
class CategoryRepository implements ApiRepositoryInterface
{
    ...
    {
        $items = $this->categoryModel::query()->with(['coverFileInfo:uuid,file_url,file_name'])->where($closure)->where([['is_show', '=', 1]])->select($this->categoryModel->searchFields)->orderByDesc('orders')->paginate((int) $perSize);
