...
class CollectionRepository implements ApiRepositoryInterface
{
    ...
    {
        $items = $this->collectionModel::query()->with(['coverFileInfo:uuid,file_url,file_name'])->with(['examCategoryInfo:uuid,title'])->where([['is_show', '=', 1]])->where($closure)->select(['uuid', 'title', 'file_uuid', 'submit_number', 'author', 'exam_category_uuid'])->orderByDesc('orders')->paginate((int) $perSize);
    ...
    {
        $bean = $this->collectionModel::query()->with(['coverFileInfo:uuid,file_url,file_name'])->where([['is_show', '=', 1]])->where($closure)->first(['uuid', 'title', 'file_uuid', 'submit_number', 'author', 'audit_author', 'level', 'content', 'exam_time']);
        if (!empty($bean)) {
            return $bean->toArray();
        }
        return [];
